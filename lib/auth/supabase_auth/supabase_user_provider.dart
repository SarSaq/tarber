...
class TarBerSupabaseUser extends BaseAuthUser {
  TarBerSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tarBerSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TarBerSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
